...
class Intent
{
	...
	public static function hasResolvedSlot($slot)
    {
        return property_exists(self::$slots, $slot) && property_exists(self::$slots->{$slot}->resolutions->resolutionsPerAuthority[0]->values[0]->value, "name");
    }
	
	public static function getResolvedSlot($slot)
    {
        if (self::hasResolvedSlot($slot)) {
            error_log($slot);
            return self::$slots->{$slot}->resolutions->resolutionsPerAuthority[0]->values[0]->value->name;
        } else {
            return null;
        }
    }
